...
public class SparqlQuery {
	...
	public float runQueryForLocationSeverity(String location) 
	{
		float total = 0;
		float weightcalc = 0;
		for(int k =1;k<=3;k++)
		 ...
		 { 
			...
			String qstr = "PREFIX act:<http://www.semanticweb.org/narmadaravalinamburi/ontologies/2020/10/accidents-ontology#>"+
			"PREFIX xsd:<http://www.w3.org/2001/XMLSchema>" 
					...
					"SELECT (COUNT(*) AS ?count) WHERE"
					    ...
					    + "FILTER regex(str(?pl_name),"  + "'" + location + "'" + ")  "+ "FILTER (?sev = " + k + ")} " ;
